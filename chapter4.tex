...
This section applies these two approaches to a simple constrained optimal control problem, comparing using the discrete and continuous squared reference error as the cost function. 
...
\begin{figure}
    ...
    \caption{Cross-track error, along-track error and total track error for a vessel trying to follow a reference trajectory. Figure adapted from \cite{prosjektoppgave}.}
...
\begin{figure}
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{$\mathbf H = \mathbf G^{-1}$}
    \end{subfigure}
    \caption{Eigenvectors of the Hessian of the cost function for the continuous cost function (top), discrete cost function (middle) and the inverse Gramian matrix (bottom) for $N=4$ and $p=2$.}

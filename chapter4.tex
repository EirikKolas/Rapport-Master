...
\end{equation}
is the unit normal to the reference path (with $\mathbf R$ a $+\pi/2$ rotation).  On straight segments \cref{eq:pxte} reduces to the standard XTE; on curved sections it approximates the TTE, yet---unlike the XTE---remains closed‐form and differentiable.
...
\end{equation}
Here $\mathbf c_\text{ref,i}$ are the control points of the reference trajectory B-spline, $B_{\mathrm{ref},i}(x)$ is the $i$-th basis function in the $N$-element basis, and $\xi_i$ is the greville abscissa of the $i$-th basis function. Replacing $\mathbf{\hat n}_{\mathrm{ref}}(t)$ in \cref{eq:pxte} with $\mathbf{\tilde n}_{\mathrm{ref}}(x)$, the integral in \cref{eq:total-track-error-b-spline} reduces to a B-spline function, which can easily be used as a cost function in the B-spline optimization framework. This approximiation has the advantage that it retains the property of being equivalent to the XTE on straight segments and while approximating the TTE on curved segments, albeit with an overestimation of the error compared to the actual PXTE. 
...
\begin{subequations}\label{eq:cost-maneuver-window}
    \begin{align}
        J_\text{ref} &= \|w_\text{mv}(x)\;\mathbf e(x)\|_{\mathbf G^{-1}}^2 \frac{1}{\text{trace}(\mathbf G^{-1})}, 
        \label{eq:cost-maneuver-window-ref}\\
        J_\text{mv} &= \|w_\text{ref}(x)\;\mathbf e'(x)\|_{\mathbf G^{-1}}^2 \frac{1}{\text{trace}(\mathbf G^{-1})},
...
\label{eq:minmpc-compact}
    \begin{aligned}
        \min
        \quad & J_\text{ref}(\mathbf{p}_\text{OS}) + J_\text{mv}(\mathbf{p}_\text{OS}) + w_\text{time}\, \mathbf t(1)
        \\
        \text{subject to}\quad
...
In this formulation:
\begin{itemize}
    \item $J_{\mathrm{ref}}$ penalizes deviation from the reference trajectory outside the encounter window (\cref{eq:cost-maneuver-window-ref}).
    \item $J_{\mathrm{mw}}$ penalizes deviation from the reference velocity outside the maneuver windows (\cref{eq:cost-maneuver-window-mv}).
